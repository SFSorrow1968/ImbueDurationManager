# -*- coding: utf-8 -*-
"""Build and release Imbue Duration Manager mod."""
import json
import subprocess
...
def main():
    ...
    print(f"\n=== Releasing Imbue Duration Manager {version} ===\n")
    ...
    run('powershell -Command "Compress-Archive -Path bin/Release/PCVR/ImbueDurationManager -DestinationPath IDM-PCVR.zip -Force"')
    run('powershell -Command "Compress-Archive -Path bin/Release/Nomad/ImbueDurationManager -DestinationPath IDM-Nomad.zip -Force"')
    ...
    run(f'gh release create {tag} IDM-PCVR.zip IDM-Nomad.zip --title "Imbue Duration Manager {version}" --notes "Release {version}"')

    print(f"\n=== Imbue Duration Manager {version} released ===\n")
