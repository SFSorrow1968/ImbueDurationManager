...
1.  **GitHub Release**: Create a release targeting tag `v<VERSION>`. Upload both zips.
    ```powershell
    gh release create v<VERSION> ImbueDurationManager_PCVR_v<VERSION>.zip ImbueDurationManager_Nomad_v<VERSION>.zip --title "v<VERSION>" --notes "See manifest.json for details."
    ```
